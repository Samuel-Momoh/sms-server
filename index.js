...
const swaggerSpec = {
  ...
  tags: [
    ...
    { name: 'App Version', description: 'Reads the production release from the Google Play Developer API and compares it against the minimum version stored in the database (no authentication required)' },
  ],
  components: {
    securitySchemes: {
      BearerAuth: {
  ...
  paths: {
    ...
    '/api/gps/app-version': {
      get: {
        tags: ['App Version'],
        security: [],
        summary: 'Check app version (query params)',
        description:
          'Reads the production release from the Google Play Developer API and compares it against the minimum version stored in the database. No authentication — the app calls this on launch, before login. ' +
          'Returns `forceUpgrade: true` if the installed version is below the database minimum, and `needsUpdate: true` if a newer version is live on the Play Store.',
        parameters: [
          { name: 'platform', in: 'query', schema: { type: 'string', enum: ['android', 'ios'], default: 'android' }, description: 'Target platform' },
          { name: 'version', in: 'query', schema: { type: 'string', example: '1.2.0' }, description: 'Currently installed app version' },
          { name: 'build', in: 'query', schema: { type: 'integer', example: 120 }, description: 'Currently installed build / versionCode' },
        ],
        responses: {
          200: {
            description: 'Version check result',
            content: { 'application/json': { schema: { $ref: '#/components/schemas/AppVersionResponse' } } },
          },
          404: { description: 'No version record configured for the given platform' },
        },
      },
      put: {
        tags: ['App Version'],
        security: [],
        summary: 'Check app version (JSON body)',
        description: 'Same logic as the GET, but accepts parameters in a JSON request body.',
        requestBody: {
          required: true,
          content: {
            'application/json': {
              schema: {
                ...
                properties: {
                  ...
                  build: { type: 'integer', example: 120 },
                },
              },
            },
          },
    ...
    '/api/gps/app-version/check': {
      post: {
        tags: ['App Version'],
        security: [],
        summary: 'Check app version (POST body)',
        description: 'Body-based form of the version check for clients that prefer POST.',
        requestBody: {
          required: true,
          content: {
            'application/json': {
    ...
    '/api/gps/app-version/minimum': {
      put: {
        tags: ['App Version'],
        summary: 'Set minimum supported version / build (admin)',
        description:
          'Stores the minimum supported version and build number for a platform. ' +
          'Any install with `version` below `minimumVersion` or `build` below `minimumBuild` gets `forceUpgrade: true` from the version check, and the app blocks until the user updates. ' +
          'Send one or both fields; fields you omit are left unchanged. Send `minimumBuild: null` to stop enforcing a build minimum.',
        security: [{ BearerAuth: [] }],
        requestBody: {
          required: true,
          content: {
            'application/json': {
              schema: {
                ...
                properties: {
                  platform: { type: 'string', enum: ['android', 'ios'], default: 'android' },
                  minimumVersion: { type: 'string', example: '1.4.0' },
                  minimumBuild: { type: 'integer', nullable: true, example: 140 },
                },
              },
            },
          },
        ...
        responses: {
          ...
          400: { description: 'Invalid platform, version or build' },
          401: { description: 'Missing or invalid credentials' },
          403: { description: 'Admin privilege required' },
        },
      },
    },
    '/api/gps/app-version/all': {
      get: {
        tags: ['App Version'],
        summary: 'List stored version records (admin)',
        description: 'Returns the database record for every platform. Does not call the Play Store.',
        security: [{ BearerAuth: [] }],
        responses: {
          200: {
              ...
              'application/json': {
                schema: {
                  ...
                  properties: {
                    ...
                    records: { type: 'array', items: { $ref: '#/components/schemas/AppVersionRecord' } },
                  },
                },
              },
            },
    ...
    '/api/gps/app-version/{platform}': {
      get: {
        tags: ['App Version'],
        security: [],
        summary: 'Check app version by platform path',
        description: 'Same as the query-based GET but with platform in the URL path.',
        parameters: [
          { name: 'platform', in: 'path', required: true, schema: { type: 'string', enum: ['android', 'ios'] }, description: 'Target platform' },
          { name: 'version', in: 'query', schema: { type: 'string', example: '1.2.0' }, description: 'Currently installed app version' },
          { name: 'build', in: 'query', schema: { type: 'integer', example: 120 }, description: 'Currently installed build / versionCode' },
        ],
        responses: {
      ...
      put: {
        tags: ['App Version'],
        security: [],
        summary: 'Check app version by platform path (JSON body)',
        description: 'Same as the body-based PUT but with platform in the URL path.',
        parameters: [
          { name: 'platform', in: 'path', required: true, schema: { type: 'string', enum: ['android', 'ios'] }, description: 'Target platform' },
        ],
        requestBody: {
  ...
  components: {
    ...
    schemas: {
      AppVersionRecord: {
        ...
        properties: {
          platform: { type: 'string', example: 'android' },
          packageName: { type: 'string', nullable: true, example: 'com.etrack.sammycodes' },
          minimumVersion: { type: 'string', example: '1.4.0' },
          minimumBuild: { type: 'integer', nullable: true, example: 140 },
          latestVersion: { type: 'string', nullable: true, example: '1.6.2' },
          latestBuild: { type: 'integer', nullable: true, example: 162 },
      ...
      AppVersionResponse: {
        ...
        properties: {
          ...
          platform: { type: 'string', example: 'android' },
          packageName: { type: 'string', nullable: true, example: 'com.etrack.sammycodes' },
          currentVersion: { type: 'string', nullable: true, example: '1.5.0', description: 'Latest version live on the Play Store (or database fallback)' },
          currentBuild: { type: 'integer', nullable: true, example: 150, description: 'Latest versionCode from the Play Developer API' },
          source: { type: 'string', enum: ['google-play-api', 'database'], description: 'Where currentVersion came from' },
          storeCheckedAt: { type: 'string', nullable: true, format: 'date-time' },
          storeError: { type: 'string', nullable: true, description: 'Why the store lookup failed, if it did' },
          minimumVersion: { type: 'string', example: '1.0.0', description: 'Minimum supported version from the database' },
          minimumBuild: { type: 'integer', nullable: true, example: 100 },
          installedVersion: { type: 'string', nullable: true, example: '1.2.0', description: 'Echo of the version the caller reported' },
          installedBuild: { type: 'integer', nullable: true, example: 120 },
          forceUpgrade: { type: 'boolean', description: 'true when installed version/build is below the database minimum — the user MUST update', example: false },
          needsUpdate: { type: 'boolean', description: 'true when a newer version is available on the Play Store — optional but recommended update', example: true },
        },
      },
    },
  },
