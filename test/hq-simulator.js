...
const client = net.createConnection({ host: HOST, port: PORT }, () => {
  ...
  console.log('📤 Sending HQ Heartbeat (*HQ,867232054850970,HTBT#)...');
  client.write('*HQ,867232054850970,HTBT#\r\n');

  // Step 2: Send GPS location (Port Harcourt / Niger Delta coordinates)
  setTimeout(() => {
    console.log('\n📤 Sending HQ GPS Location (*HQ,867232054850970,V1,210046,A,0453.2956,N,00654.7924,E,0.00,0,)...');
    client.write('*HQ,867232054850970,V1,210046,A,0453.2956,N,00654.7924,E,0.00,0,\n');
  }, 500);

  // Step 3: Send fragmented GPS packet
  setTimeout(() => {
    console.log('\n📤 Sending Fragmented HQ Packet...');
...
client.on('data', (data) => {
  console.log(`📥 Server response received: ${data.toString()}`);
});
